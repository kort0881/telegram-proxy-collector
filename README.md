...
<!-- AI_ANALYTICS_START -->
*Отчёт сгенерирован 2026-09-22 07:23 UTC*

## Отчёт о состоянии прокси‑списков  
*Дата и время: 2026‑09‑22 14:35 UTC*

- **Всего прокси в истории:** 158  
- **Качество:** все 158 прокси находятся в категории «хорошие» (пинг < 1.5 с).  
- **Регионы:** `eu` — 150, `ru` — 8.  
- **Типы:** `mtproto` — 36, `socks5` — 122.  
- **Источники:** данные отсутствуют.

### Рекомендации  
- **Регион:** для большинства задач предпочтительно использовать европейские (`eu`) прокси, так как их большинство и они находятся в категории «хорошие». Российские (`ru`) прокси могут пригодиться при необходимости локального доступа.  
- **Тип:** `socks5` обеспечивает более широкую совместимость и сейчас покрывает 77 % всех прокси, поэтому рекомендуется использовать их в первую очередь. `mtproto` пригодятся, если требуется специфическая поддержка Telegram.

### Аномалии  
Плохих прокси (пинг > 5 с) не обнаружено, аномалий нет.
<!-- AI_ANALYTICS_END -->
